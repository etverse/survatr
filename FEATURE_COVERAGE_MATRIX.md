...
| `type = "rmtl"` (restricted mean time lost) | 🟢 | `test-estimands-rmtl.R` | `RMTL(t) = t - RMST(t)` identity to 1e-12; inherits the RMST oracle pins (survRM2, delicatessen) transitively. Wired across gcomp / IPW / IPCW / Track B (shared SE filler); sandwich vs bootstrap SE ratio in (0.7, 1.4). |
...
| `type = "quantile"` (survival quantile / median) | 🟢 | `test-estimands-quantile.R` | Constant-hazard closed form: median `= log(2)/λ`, `τ_q = -log(1-q)/λ` matched to ~0.3% at n = 20000 (estimator returns the exact linear-interp crossing of the fitted curve; verified at n up to 1e5). `survival::survfit()` KM-median sanity. Vector `q` supported. |
