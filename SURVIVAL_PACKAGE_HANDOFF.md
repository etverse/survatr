...
| 6 | ✅ `ad02fe6` | [CHUNK_6_ICE_B.md](CHUNK_6_ICE_B.md) | Track B (ICE-hazards): per-step hazard target + survival-tail pseudo-outcome (`Ỹ_k = D_k + (1−D_k)q_{k+1}`); survatr owns the backward loop + cross-step IF cascade with the `(1−D_k)` failure carry-forward, reusing causatr's **single-model** primitives (`ice_fit_step`, `ice_if_setup`, `correct_model`). `confounders_tv` + `history` API. Validated vs forward-sim truth, `delicatessen` sandwich (~1e-5), bootstrap, `lmtp`. | 3, causatr ICE |
